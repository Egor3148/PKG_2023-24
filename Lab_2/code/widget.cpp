    ...
    , ui(new Ui::Widget)
{
    ...
    connect(ui->fileTreeView,SIGNAL(doubleClicked(QModelIndex)), this, SLOT(on_listView_doubleClicked));
...
void Widget::on_lineEdit_editingFinished(const QModelIndex &index)
{
    QListView* listView = (QListView*)sender();
    QFileInfo fileInfo = model->fileInfo(index);
...
void Widget::on_multiChoice_clicked()
{
    ...
    twInfo->setHorizontalHeaderItem(2, new QTableWidgetItem("Format"));
