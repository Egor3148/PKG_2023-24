...
plane::plane(type t, QWidget *parent ): QWidget(parent)
{
    ...
    if (currentFilePath[currentFilePath.size() - 1] != QChar('t') ||
        currentFilePath[currentFilePath.size() - 2] != QChar('x') ||
        currentFilePath[currentFilePath.size() - 3] != QChar('t') ||
    ...
    for(int i = 0; i<n;++i)
    {
        ...
        p2.setX(x2);
        p2.setY(y2);
        cropper.push_back(qMakePair(p1,p2));
    }
    this->t = t;
    if(t == type::Central)
    {
...
//gets the parameter of the line
...
float mulScalar(QPair<QPointF,QPointF> v1, QPair<QPointF,QPointF> v2)
{
...
void plane::ClipByCirus(QPair<QPointF, QPointF> segment){
    ...
    {
        t = getT(cropper[i], segment, &onLine);
        if(onLine){
            T_enter.push_back(getParameterOfPoint(cropper[i].first, segment));
            T_outer.push_back(getParameterOfPoint(cropper[i].second, segment));
            T_outer.push_back(getParameterOfPoint(cropper[i].first, segment));
            T_enter.push_back(getParameterOfPoint(cropper[i].second, segment));
            onLine = false;
            continue;
        }
        S=-mulScalar(cropper[i], segment);
        if(t>=0 && t<=1){
            if(S>0){
                T_enter.push_back(t);
            }else if(S<0)
