    ...
    : QWidget(parent)
{
    ...
    Slider4 = nullptr;  //This is a slider for "Black" in CMYK (it's the only system with dim = 4)
    SliderLabel4 = new QLabel;
    SliderLabel4->setMaximumSize(30,30);
    SliderLabel4->setAlignment(Qt::AlignRight);

    /******************Initializing color display*********************************/
    ...
    /*******************Signals for refreshing the data when sliders are moved****************************/
    ...
    /*********************Signals for refreshing data wher spinboxes' values are changed******/
    ...
    /*********************Adding the names of color systems*****************/
    LeftCB->addItem("RGB");
    LeftCB->addItem("CMYK");
    LeftCB->addItem("HSV");
    LeftCB->addItem("HLS");
...
/******************Color converters************************/
double Widget::RGB_To_XYZ(double arg)
{
    double x=arg;
    if(x>= 0.04045)
    {
...
double Widget::XYZ_To_LAB(double arg)
{
    double x=arg;
    if(x>= 0.008856)
    {
...
double Widget::LAB_To_XYZ(double arg)
{
    double x = arg;
    if(pow(x,3)>= 0.008856)
    {
...
double Widget::XYZ_To_RGB(double arg)
{
    double x=arg;
    if(x >= 0.0031308)
    {
...
/**********************************Getting the values of the chosen color**********************/
QVector<int> Widget::getValues(space_name value_id)
{
    QVector<int> values;
    switch(value_id)
    {
    /******QColor supports getting RGB, CMYK, HSV and HLS values automatically, so no convertions required. Learn more in QT documentation****/
    case space_name::rgb:
    {
...
/***********************************Sets a new color via processing input values******************/
...
/**********************Sets new names for spinboxes according to a chosen option in combo boxes********/
...
/***********************Defining the ranges of input tools********************************/
